...
if (userrole !== "ADMIN" && userrole !== "TECHNICIAN") {
  document.getElementById('mstHideContainer').style.display = 'none';
}
...
document.addEventListener("DOMContentLoaded", function () {
  const table = $("#inspectionId").DataTable({
    ajax: {
      url: `${api}/mstForm/`, // Replace with your API endpoint
      dataSrc: "",
      headers: {
        'Authorization': `${token}`}
    },
        ...
        render: function (data, type, row) {
          ...
          return `
                        <button class = "export" onclick="exportInspectionForm(${row.id})">Export</button>
                         <button class="preview" onclick="previewMstForm(${row.id})">Preview</button>
                    `;
          }else if(userrole==="ADMIN"){
            return `
  ...
  window.exportInspectionForm = function (id) {
    ...
    const url = `${api}/mstForm/${id}/excel`;
    window.location.href = url;
  };
  window.previewMstForm = function(id) {
    const url = `${api}/mstForm/${id}/excel?token=${encodeURIComponent(token)}`;
...
function sendInspectionForm(id, isAccepted, approverId, token) {
  ...
  const url = new URL(`${api}/mstForm/${id}/action`);
